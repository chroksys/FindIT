...
import { ChartBar, Users, Eye, ArrowUpRight, ArrowDownRight } from '@phosphor-icons/react';
  ...
  return (
    <div className="container section" style={{ display: 'flex', flexDirection: 'column', gap: 'var(--spacing-xlarge)' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: 'var(--spacing-large)', marginBottom: 'var(--spacing-xlarge)' }}>
        <StatCard title="Total Ticket Revenue" value="UGX 0" change="+0%" isPositive={true} icon={<ChartBar size={24} />} delay="0.1s" />
        <StatCard title="Total Tickets Sold" value="0" change="+0%" isPositive={true} icon={<ChartBar size={24} />} delay="0.2s" />
        <StatCard title="Page Views" value="0" change="+0%" isPositive={true} icon={<Eye size={24} />} delay="0.3s" />
        <StatCard title="Total Followers" value="0" change="+0%" isPositive={true} icon={<Users size={24} />} delay="0.4s" />
      </div>
